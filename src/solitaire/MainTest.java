			...
			for(int i = 0; i < 7; i++)
			{
				...
				if(cartesVisibles.size() == 1)
				{
					...
					for(int m = 0; m < 7 && m != i; m++)
					{
						...
						int indexCarteVisible = ReferentielCarte.getIndexInValeursCartes(carteVisible.getValeur());
						
						int indexDerniereCarteVisible = ReferentielCarte.getIndexInValeursCartes(derniereCarteVisible.getValeur());
				...
				/*for(int j = 0; j < cartesVisibles.size(); j++)
				{
					...
					if (pile.getCartes().isEmpty()) 
					{
						...
						// Ex: pour une carte de valeur As, cette variable vaut 0, pour un 2, elle vaut 1 et ainsi de suite 
						int indexCarteVisible = ReferentielCarte.getIndexInValeursCartes(carte.getValeur());
						
						int indexDerniereCartePile = ReferentielCarte.getIndexInValeursCartes(derniereCartePile.getValeur());
